...
# cmd line arg 4 will be feedback / transfer
if num_cmd_line_arguments >= 4:
    if sys.argv[3] == 'Feedback':
        input_feedback = ['Feedback', 'Transfer']
    else:
        input_feedback = ['Transfer', 'Feedback']
else:
    input_feedback = ['', 'Feedback', 'Transfer']

# hard code feedback duration to 15 min
input_feedback_condition = '15min';
...
while expInfo['feedback_on'] not in [ 'Transfer','Feedback']:
    expInfo['feedback_on'] =  input_feedback
    dlg = gui.DlgFromDict(dictionary=expInfo, title=expName, 
        labels = {'participant': 'Participant ID (###)', 
                  'run': 'Run', 
                  'feedback_on': 'Transfer/Feedback',
                  'anchor': 'Participant Anchor (transfer01 only)',
                  'feedback_condition': '15 min'},
...
if expInfo['feedback_on']=='Feedback':
    filename = f"data/sub-sharp2{expInfo['participant']}" + os.path.sep + 'sub-sharp2%s_ses-neurofeedback_task-neurofeedback_run-0%s' %(expInfo['participant'],expInfo['run'])
elif expInfo['feedback_on']=='Transfer':
    filename = f"data/sub-sharp2{expInfo['participant']}" + os.path.sep + 'sub-sharp2%s_ses-neurofeedback_task-transfer_run-0%s' %(expInfo['participant'],expInfo['run'])
...
#with open(filename+'_roi_outputs.csv', 'a') as f_tsv:
    #stim_writer = csv.writer(f_tsv, delimiter=',', quotechar='|', quoting=csv.QUOTE_MINIMAL)
    #stim_writer.writerow(['volume','scale_factor', 'onset','duration',
       #          'cen_signal', 'dmn_signal','trial_type', 'cen_hit', 
        #         'dmn_hit', 'pda', 'ball_y_position'
         #       'top_circle_y_position', 'bottom_circle_y_position', 'feedback_on',
          #      'slider_noting', 'slider_ballcheck', 'slider_difficulty', 'slider_calm'])   # revert to original    
# write bids tsv header row
with open(filename+ '_roi_outputs.tsv', 'a') as f_tsv: 
    stim_writer = csv.writer(f_tsv, delimiter='\t', quotechar='|', quoting=csv.QUOTE_MINIMAL)
...
slider_instruction = visual.TextStim(win=win, ori=0, name='text', # first slide of text?
        text="你会看到一个滑动评分条。\n\n请用左手无名指点击向左，中指点击向右滑动，\n\n然后食指确认。\n\n如果明白上述指令，请按任意键继续。", font=u'SimSun',
...
text = visual.TextStim(win=win, ori=0, name='text', 
    text=u'注意练习\n\n接下来你会看到%s个圆圈。\n\n如果注意练习成功，中间的白色小球会向上方的黄色圆圈移动。\n\n尽量让白色小球在上方的黄色圆圈内停留５秒。\n\n５秒之后，黄色圆圈会变小，白色小球会回到原处。\
\n\n这一轮训练会持续２分钟。\n\n 按任意键继续。' %int(roi_number),font=u'SimSun',
    pos=[-1, 0], height=0.1, wrapWidth=1.2,
    color=u'white', colorSpace='rgb', opacity=1,
    depth=0.0)
...
text_4 = visual.TextStim(win=win, ori=0, name='text_4',
    text='default text',    font=u'SimSun',
...
def run_instructions(instruct_text):
    instruct_text.draw()
    win.flip()
    wait_for_keypress(['1','2','3','4']) #this determines which keys progress text to next slide
...
instruct_text = visual.TextStim(win=win, ori=0, name='instruct_text', #this is the code chunk that manipulates instructions text centering
    text=u'replace me', font=u'SimSun',
    pos=[0, 0], height=0.1, wrapWidth=1.2,
    color=u'white', colorSpace='rgb', opacity=1,
    depth=0.0)
...
no_feedback_run1_text = f"接下来，你会开始注意练习。\
\n\n你选择了{expInfo['anchor']}作为你的关注点。 \
\n\n你可以继续用这个关注点，\n\n 但也可以换一个关注点。\
\n\n你会看到两个圆圈和一个白色小球，\
\n\n但是这一轮练习中它们不会移动。\
\n\n 按任意键继续。"

ready_text="在前30秒，你会看到屏幕当中有一个十字(+)。\
\n\n当你看到十字的时候，你不需要进行注意练习，\n\n请放松休息。\
\n\n当你看到圆圈出现，请开始注意练习。\
\n\n这一轮训练会持续２分钟。\n\n 按任意键继续。" 

feedback_run1_text1 = "很好!下一轮开始，你将会得到注意练习的反馈。\
\n\n你会看到两个圆圈和一个白色小球。\
\n\n如果注意练习成功，\n\n中间的白色小球会向上方的黄色圆圈移动。\
\n\n反之，可能会向下方的蓝色圆圈移动。\
\n\n白色小球在任意圆圈内停留5秒hou会回到原处。 \
\n\n尽量让白色小球向上方的圆圈移动！\n\n按任意键继续。"

feedback_run1_text2 = "尽量不要时刻关注小球的移动情况，\n\n因为这可能干扰你注意练习的效果。\
\n\n尽量将注意力放在你自身的感官变化上。\
\n\n小球的移动情况只是你注意练习的反馈参考。\
\n\n这一轮训练会持续２分钟。\n\n按任意键继续。" 

feedback_later_runs_text = "很好! \n\n接下来，你将继续完成2分钟的注意练习\
\n\n当屏幕上出现十字(+)时，请放松休息。\n \n 当圆圈出现，尽力让白色小球向上移动! \
\n\n这一轮训练会持续２分钟。\n\n 按任意键继续。"

no_feedback_later_runs_text = "很好!\n\n 接下来，你将继续完成2分钟的注意练习。\
\n\n这一轮练习中小球不会移动，所以你不需要关注它。\n\n 按任意键继续。"

# Depending on whether feedback is offered/which run it is -- show different instruction slides
if expInfo['feedback_on'] == "Transfer":
    if int(expInfo['run']) == 1: 
...
while continueRoutine:
    ...
    if key_resp_3.status == STARTED:
        theseKeys = event.getKeys(keyList=['t','+','5', 5,'s','S'])
...
while continueRoutine and routineTimer.getTime() > 0:
    ...
    # a list of [CEN, DMN] for the current frame
        ...
        with open(filename+ '_roi_outputs.tsv','a') as tsv_file:
            stim_writer = csv.writer(tsv_file, delimiter='\t')
            stim_writer.writerow([triggerClock.getTime(),triggerClock.getTime() + 1.3, 'feedback', frame, roi_raw_activations[0], 
            roi_raw_activations[1], pda_outlier, ball.pos[1], hit_counter[0], hit_counter[1], expInfo['scale_factor'], target_circles[0].pos[1], 
...
run_slider(question_text='你刚才在做注意练习吗？',
                left_label='一直没做', right_label='一直在做')
run_slider(question_text='你刚才在注意小球的位置吗？',
                left_label='一直没注意', right_label='一直在注意')
run_slider(question_text='刚才的注意练习困难吗？',
                left_label='一点都不难', right_label='非常困难')
run_slider(question_text='你现在心情平静吗？',
                left_label='一点都不平静', right_label='非常平静')

# convert csv output to BIDS-format tsv
bids_tsv_convert_balltask(infile = f'{filename}_roi_outputs.csv') 
print('ran bids converter')
...
if expInfo['run'] == '1' and expInfo['feedback_on'] == 'Transfer':
    next_run =1
    next_feedback= 'Feedback'
elif expInfo['run'] == '2' and expInfo['feedback_on'] == 'Transfer': 
    next_run =6
    next_feedback = 'Feedback'
elif expInfo['run'] == '5':
    next_run =2
