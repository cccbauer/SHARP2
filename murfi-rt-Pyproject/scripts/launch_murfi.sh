...
then
	...
	--separator=" " \
	--add-combo="Step" --combo-values "setup|---------LOCALIZER---------|2vol|resting_state|extract_rs_networks|process_roi_masks|register|experience_sampling|---------FEEDBACK---------|2vol|register|transfer_pre|feedback|transfer_post|cleanup" \
	--cancel-label "Exit" --ok-label "Run Selected Step")
	ret=$?
	# parse zenity output using space as delimiter
	read -a input_array <<< $input_string
...
if [ "$step" == 'create' ]; then
    ...
    if [ -d "$subj_dir" ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            echo "User chose not to overwrite."
            exit 0
        fi
    ...
    # Handle feedback or other steps
